...
function App() {
  ...
  // Проверяем, давал ли пользователь согласие на Cookie ранее
  useEffect(() => {
    const isCookieAccepted = localStorage.getItem('shpagin_cookie_accepted');
    if (!isCookieAccepted) {
      // Показываем плашку через 2 секунды после захода на сайт, чтобы это выглядело плавно
  ...
  return (
    <div className="min-h-screen bg-anthracite-900 text-white font-sans relative">
      ...
      {/* Всплывающее уведомление о Cookies и Политике конфиденциальности */}
      <AnimatePresence>
        {showCookieNotice && (
          <motion.div
...
function BookingSection({ onPrivacyClick }: { onPrivacyClick: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const TELEGRAM_CHAT_ID = "939290757"; // Обновленный ID мастера
...
function ReviewsSection() {
  ...
  const reviews = [
    ...
    },
    {
      name: 'Сергей Ф.',
      rating: 5,
      text: 'Отличный шиномонтаж! Всегда быстро и качественно меняю здесь резину. Masters работают аккуратно, диски целые, балансировка идеальная. Рекомендую!',
      date: '7 ноября 2025',
    },
    {
        ...
        >
          <div className="bg-anthracite-900 rounded-2xl overflow-hidden border border-anthracite-700">
            ...
            <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <a
                href="https://yandex.ru/maps/?rtext=~55.845946,38.423984"
                target="_blank"
        ...
        >
          <div className="bg-anthracite-900 rounded-2xl overflow-hidden border border-anthracite-700">
            ...
            <div className="p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              {/* Универсальная ссылка на Яндекс.Карты с координатами твоего шиномонтажа в Клюшниково */}
              <a
                href="https://yandex.ru/maps/?rtext=~55.845946,38.423984"
                target="_blank"
                rel="noopener noreferrer"
    ...
    >
      ...
      >
        ...
        <div className="p-6 text-anthracite-300 leading-relaxed space-y-4 text-sm">
          ...
          <div className="border-t border-anthracite-700 pt-4">
            <h4 className="font-bold text-white mb-2">9. Contact Информация</h4>
