...
function EvakuatorPage() {
  ...
  return (
    <div className="min-h-screen bg-anthracite-900 text-white font-sans selection:bg-accent-500 selection:text-white">
      {/* Главный экран / Hero Section */}
      <section className="relative min-h-screen w-full flex items-center justify-center overflow-hidden py-12 border-b border-anthracite-800">
        <img
          src="/111.jpg"
          alt=""
          className="absolute inset-0 w-full h-full object-cover object-center md:bg-fixed"
          style={{ opacity: 0.4 }}
        />
      ...
      <section className="py-20 md:py-32 bg-anthracite-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-black uppercase mb-4">Наши преимущества</h2>
            <div className="w-24 h-1 bg-accent-500 mx-auto mb-6" />
            <p className="text-anthracite-400 max-w-2xl mx-auto">
              Быстрая подача спецтехники и бережная транспортировка вашего автомобиля в любой ситуации.
            </p>
          </motion.div>
            ...
            {[
              {
                ...
                title: 'Подача от 15–30 минут',
                desc: 'Машина дежурит вблизи транспортной развязки автодороги А107 (малой Бетонки) и М7 (Горьковского шоссе), между Ногинском и Электрогсталью, обеспечивая моментальный выезд после звонка.'
              },
              {
                icon: <MapPin className="w-6 h-6" />,
                title: 'Межгород и внутригородские перевозки',
                desc: 'Наши эвакуаторы IVECO одинаково идеально приспособлены и для движения по узким городским улочкам и для быстрых перевозок на дальние расстояния'
              },
              {
                icon: <ShieldCheck className="w-6 h-6" />,
                title: 'Безопасная погрузка и бережная перевозка',
                desc: 'Профессиональное оборудование и большой опыт гарантируют полную сохранность вашего ТС от повреждений.'
              }
            ].map((item, idx) => (
              <motion.div
                key={idx}
                whileHover={{ y: -6 }}
                className="p-8 rounded-2xl bg-anthracite-900 border border-anthracite-700/60 transition-all"
              >
                <div className="w-12 h-12 rounded-xl bg-accent-500/10 flex items-center justify-center text-accent-500 mb-6">
                  {item.icon}
                </div>
                <h3 className="text-xl font-bold mb-3 text-white">{item.title}</h3>
                <p className="text-anthracite-400 text-sm leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Блок цен и базовых услуг */}
      <section className="py-20 bg-anthracite-900 border-t border-anthracite-800">
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-black uppercase mb-4">Стоимость услуг</h2>
            <div className="w-24 h-1 bg-accent-500 mx-auto" />
          </div>
          ...
          <p className="text-xs text-anthracite-500 text-center mt-4 leading-relaxed">
            * Итоговая цена зависит от сложности погрузки (заблокированные колеса, повреждения рулевого управления) и расстояния. Точную сумму мастер назовет по телефону.
          </p>
        </div>
      </section>

      {/* Информационный SEO-футер для Яндекс Карт */}
      <section className="py-12 px-4 bg-anthracite-950 border-t border-anthracite-800/80 text-center">
